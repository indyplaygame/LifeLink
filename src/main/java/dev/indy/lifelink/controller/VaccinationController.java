...
public class VaccinationController {
    ...
    @AuthRequired
    @GetMapping("/{id}")
    public ResponseEntity<Vaccination> get(@PathVariable Long id, HttpSession session) {
        try {
            Vaccination vaccination = this._vaccinationService.getVaccination(session, id);
    ...
    public ResponseEntity<Vaccination> update(
        ...
        @Validated(ValidationGroups.OnUpdate.class) @RequestBody AddVaccinationRequest body,
        HttpSession session
    ) {
        try {
            Vaccination vaccination = this._vaccinationService.updateVaccination(session, id, body);
            return ResponseEntity.ok(vaccination);
        } catch(EntityNotFoundException e) {
            throw new HttpException(HttpStatus.NOT_FOUND, e.getMessage());
        }
    ...
    public ResponseEntity<Void> delete(@PathVariable Long id, HttpSession session) {
        try {
            this._vaccinationService.deleteVaccination(session, id);
