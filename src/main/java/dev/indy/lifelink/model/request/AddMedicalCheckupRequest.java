...
public record AddMedicalCheckupRequest(
    @NotBlank(groups = ValidationGroups.OnCreate.class, message = "Checkup details must not be empty")
    @Length(max = 2000, message = "Checkup details cannot exceed 2000 characters")
    String details,

    @NotBlank(groups = ValidationGroups.OnCreate.class, message = "Checkup date must not be empty")
